...
@app.route('/sent_messages/<recipient_user_id>', methods=['GET'])
def get_sent_messages_to_recipient(recipient_user_id):
    """Get messages sent by the authenticated user to a specific recipient"""
    try:
        # Get sender's password from header for authentication
        sender_password = request.headers.get('sender_password') or request.headers.get('Sender-Password')
        sender_user_id = request.headers.get('sender_user_id') or request.headers.get('Sender-User-Id')
        
        if not sender_password:
            return jsonify({
                'error': 'sender_password header is required'
            }), 400
            
        if not sender_user_id:
            return jsonify({
                'error': 'sender_user_id header is required'
            }), 400
        ...
        # Get messages sent by the authenticated user to the specified recipient
        cursor.execute('''
            SELECT id, sender_user_id, recipient_user_id, message, timestamp, is_read
            FROM messages 
            WHERE sender_user_id = ? AND recipient_user_id = ?
        ...
        return jsonify({
            'error': f'Failed to fetch sent messages: {str(e)}'
        }), 500
